                        ...
                        outDigest:(NSData *_Nonnull *_Nullable)outDigest
{
    ...
    *outKey = [attachmentKey copy];
    ...
    if (cryptStatus != kCCSuccess) {
        NSLog(@"%@ %s CCCrypt failed with status: %d", self.tag, __PRETTY_FUNCTION__, (int32_t)cryptStatus);
        free(buffer);
        return nil;
    }
    ...
    NSLog(@"%@ computed hmac: %@", self.tag, hmac);
    ...
    *outDigest = [self computeSHA256Digest:encryptedPaddedData];
    NSLog(@"%@ computed digest: %@", self.tag, *outDigest);
